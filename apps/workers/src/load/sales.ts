// =============================================================================
// Sales Load Worker — Calls load_payments stored procedure
...
/**
 * Load payments from a completed extraction run's raw.jsonl.
 * Calls load_payments stored procedure (DR-043).
 */
export async function loadSales(
  ...
  logger.info({ extractionRunId, recordCount: records.length }, 'Starting sales load');

  const programResult = await db.query(
    `INSERT INTO audit_program_runs (tenant_id, program_name, args, status)
     VALUES ($1, 'load_payments', $2, 'running')
  ...
  try {
    await db.query('SELECT load_payments($1, $2)', [extractionRunId, tenantId]);

    await db.query(
      `UPDATE audit_program_runs
       SET run_end = NOW(), status = 'completed', output_summary = $2
       WHERE program_run_id = $1 AND tenant_id = $3`,
      [programRunId, `Loaded ${records.length} payments`, tenantId]
    );

    logger.info({ programRunId }, 'Sales load completed');
  } catch (error) {
