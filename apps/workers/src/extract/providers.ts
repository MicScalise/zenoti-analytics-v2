// =============================================================================
// Provider (Employee) Extraction Worker — Fetches employees from Zenoti API
// Implements: TASK-024, REQ-EXT-01
// Design: api-extraction-specification.md §2.12
...
/**
 * Extract employees (providers) for all enabled centers.
...
export async function extractProviders(
  ...
  const entityType: EntityType = 'employees';
  ...
  for (const center of centers) {
    ...
    try {
      ...
      logger.info({ runId, centerId: center.zenoti_location_id }, 'Fetching employees');
      const employees = await client.getEmployees(center.zenoti_location_id);

      const filePath = `${RAW_DATA_DIR}/${tenantId}/${entityType}/${runId}/raw.jsonl`;
      const checksum = await writeRawJsonl(filePath, employees);

      await completeExtractionRun(
        db, runId, tenantId, employees.length, 0, filePath, checksum
      );

      logger.info(
        { runId, recordsFetched: employees.length },
        'Provider extraction completed'
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      await failExtractionRun(db, runId, tenantId, message);
      logger.error({ runId, error: message }, 'Provider extraction failed');
      throw error;
    }
  }
}
